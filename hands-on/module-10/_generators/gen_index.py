...
<body>
  ...
  <div class="container">
    ...
    <div class="callout warn">
      ...
      only the <strong>standard library</strong>. This is the <strong>course finale</strong> (Lab 5.2): you
      practice both halves of responsible AI &mdash; <strong>treat input as data</strong> (prompt injection),
      ...
      loop. The agent-assembly labs reuse the <strong>LangChain-shaped shim</strong> from Modules 6&ndash;9
      (same names &amp; shapes), driven by a deterministic scripted model &mdash; so there are <strong>no API
      keys, no model downloads, no network</strong>. Each Advanced lab ends with an <em>optional</em>,
      non-graded cell that runs the <strong>real LangChain</strong> (<code>pip install langchain
      langchain-ollama</code> + <code>ollama run llama3.2:1b</code>, or <code>ChatGroq</code> with a
      <code>GROQ_API_KEY</code>); it skips cleanly if absent.
    </div>
