...
WORK = os.path.join(os.environ.get("TEMP") or os.environ.get("TMP") or "/tmp", "biaa-lab-04-{nn:02d}")
...
Here "**positive**" means the model **flags the condition** (disease / fraud / attack). The same four
outcomes carry very different real-world stakes:

| Outcome | Health | Finance | Cybersecurity |
|---|---|---|---|
| **TP** &mdash; caught a real one | Flags a real tumour &rarr; treatment starts early | Blocks a truly stolen card &rarr; fraud prevented | Flags a real intruder &rarr; breach stopped |
| **TN** &mdash; correct all-clear | Clears a healthy scan &rarr; no needless biopsy | Approves a genuine purchase &rarr; smooth checkout | Lets a real employee in &rarr; work uninterrupted |
| **FP** &mdash; false alarm | Flags a healthy patient &rarr; needless biopsy, cost &amp; fear | Declines a genuine card &rarr; customer stranded, churns | Locks out a real employee &rarr; blocked mid-deploy |
| **FN** &mdash; missed one (usually worst) | Calls a real tumour "healthy" &rarr; cancer goes untreated | Approves a stolen-card charge &rarr; fraud loss booked | Waves an attacker through &rarr; silent breach |
