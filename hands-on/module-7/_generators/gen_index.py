...
<body>
  ...
  <div class="container">
    <div class="callout">
      ...
      Each lab writes to its own temp working dir (<code>biaa-lab-07-NN/</code>). Stuck? Every lab has a full
    ...
    <div class="callout warn">
      ...
      <code>biaa-venv</code>). You build the <strong>email-drafting agent</strong> (the client&rsquo;s
      Lab 4.1) piece by piece: the automation pipeline, the three patterns
      (<strong>draft &middot; extract &middot; route</strong>), structured output, validation, retries &amp;
      idempotency, and the <strong>draft-not-send</strong> guardrail. The agent-assembly labs bind real
      <code>@tool</code> functions with <code>create_agent</code>. The <strong>graded</strong> cells assert
      only on the deterministic parts you build (tool wiring, prompt formatting, the pipeline, the
      gather-only guardrail) and <strong>never call an LLM</strong>, so they verify offline with
      <strong>no API keys and no network</strong>. Cells marked <em>Optional &mdash; run it for real</em>
      call a live local model (<code>ollama run llama3.2:1b</code>, or <code>ChatGroq</code> with a
